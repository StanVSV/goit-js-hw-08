...
const handleGalleryClick = event => {
    ...
    const alt = target.alt
        ...
        lightbox = basicLightbox.create(`
            <img src="${largeImageSource}" alt="${alt}">
        `, {
            onShow: () => {
                document.addEventListener('keydown', handEscClick) 
            },
            onClose: () => {
                document.removeEventListener('keydown', handEscClick)
            }  
        });

        lightbox.show();
}
const handEscClick = event => {
